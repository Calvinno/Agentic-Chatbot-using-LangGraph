...
llm = ChatGoogleGenerativeAI(model="gemini-3.6-flash",temperature=0, api_key=GOOGLE_API_KEY)
...
def ingest_rag_document(file_path):
    DB_PATH = "faiss_index"  # On utilise le même nom de dossier
    ...
    print(f"Début de l'embedding pour {len(chunks)} chunks...")

    for i in range(0, len(chunks), taille_lot):
        lot_actuel = chunks[i : i + taille_lot]
        
        if vector_store is None:
            vector_store = FAISS.from_documents(lot_actuel, embeddings)
        else:
            # Note: FAISS.add_documents attend généralement une liste de documents
            vector_store.add_documents(lot_actuel)
            
        print(f"Progression : {min(i + taille_lot, len(chunks))} / {len(chunks)} chunks traités.")
        
        # Pause longue pour respecter les limites de l'API
        time.sleep(15) 

    vector_store.save_local(DB_PATH)
    print(f"Terminé et sauvegardé avec succès dans le dossier '{DB_PATH}' !")


def get_retriever():
    DB_PATH = "faiss_index"  # Doit correspondre à la sauvegarde ci-dessus
    
    # get_retriever se contente de charger la base déjà calculée
...
def get_stock_price(symbol: str) -> dict:
    ...
    """
    url = f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={symbol}&apikey=3VD5X7UME50TZ001"
    r = requests.get(url)
    return r.json()
...
# Make tool list
tools = [search_tool,calculator, get_stock_price,get_current_weather, purchase_stock]
