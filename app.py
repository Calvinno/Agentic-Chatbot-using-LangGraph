...
def get_clean_text(content):
    """Extrait proprement le texte, peu importe comment il est formaté."""
            ...
            def resumed_ai_only_stream():
                for message_chunk, metadata in chatbot.stream(
                    ...
                    if isinstance(message_chunk, AIMessage):
                        text_chunk = get_clean_text(message_chunk.content)
                        if text_chunk:
                            yield text_chunk
...
for message in st.session_state["message_history"]:
    with st.chat_message(message["role"]):
        clean_text = get_clean_text(message["content"])
...
if current_thread_has_pending_hitl:
    ...
    with approve_column:
        if st.button("✅ Approve Purchase", key=f"approve_{st.session_state['thread_id']}", type="primary", use_container_width=True):
            resume_hitl_execution("yes")
    with reject_column:
        if st.button("❌ Reject Purchase", key=f"reject_{st.session_state['thread_id']}", use_container_width=True):
    ...
    with st.chat_message("assistant"):
        ...
        def ai_only_stream():
            ...
            if pending_interrupt is not None:
                ...
                yield "\n\n⚠️ This action requires your approval. Use the buttons below."
